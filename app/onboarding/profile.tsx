...
export default function FaithConnectOnboarding() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const profileData: any = { name, faith, bio, username, avatar: avatar || undefined };
      ...
      Toast.show({ type: 'error', text1: 'Error saving profile', text2: 'Please try again' });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white">
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <ScrollView className="flex-1 px-6 pt-2">
          ...
          {isLeader && currentStep === 4 && renderStep5Leader()} {/* Avatar for leaders */}
          {isLeader && currentStep === 5 && renderStep6Leader()} {/* Content Focus for leaders */}
          {isLeader && currentStep === 6 && renderStep7Leader()} {/* Audience for leaders */}
          {!isLeader && currentStep === 4 && renderStep4Regular()} {/* Avatar for regular users */}
        </ScrollView>
