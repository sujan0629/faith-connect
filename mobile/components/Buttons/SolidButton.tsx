...
export const SolidButton = ({ label, onPress, variant = 'primary', style, loading = false }: Props) => {
  ...
  const textStyles = variant === 'primary' || variant === 'blue' ? 'text-white' : variant === 'secondary' ? 'text-gray-600' : 'text-slate-200'
  ...
  return (
    <Pressable onPress={loading ? undefined : onPress} className={`${base} ${styles}`} style={buttonStyle}>
      {loading ? (
        <ActivityIndicator size="small" color={variant === 'primary' || variant === 'blue' ? 'white' : '#5F9598'} />
      ) : (
